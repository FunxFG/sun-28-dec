...
export class TMPAutoPopulator {
  ...
  async fetchTrafficAssessment(address, roadData) {
    try {
      ...
      const response = await fetch(
        `https://tmp-generator-1.preview.emergentagent.com/api/traffic-assessment?lat=${lat}&lng=${lng}&address=${encodeURIComponent(address)}`
      );
  ...
  async fetchSiteAssessment(address, roadData) {
    try {
      ...
      const response = await fetch(
        `https://tmp-generator-1.preview.emergentagent.com/api/site-assessment?lat=${lat}&lng=${lng}&address=${encodeURIComponent(address)}`
      );
